...
本研究では部分空間法の前処理に対して，計算時間を高速化する手法を提案する．部分空間法を適用することで，シミュレーションは大幅に高速化することができるが，前処理に膨大な計算時間がかかることが課題である．そこで本研究は，前処理をシミュレーション時間に対して分割し，前処理全体の計算時間を削減する手法を提案する．計算機実験の結果，前処理の計算時間のボトルネックとなっている基底計算のための主成分分析の高速化を確認し，前処理の高速化に有効であると示すことができた．また，計算結果の精度について，基底の累積寄与率や相対誤差を用いて評価をし，分割数によるシミュレーションのレンダリング結果の変化を確認した．
...
\begin{equation}
\frac{\partial  \bm{p} }{\partial x} \approx \frac{p_{i+1,j,k} - p_{i,j,k}}{\Delta x}, \quad
\frac{\partial  \bm{p} }{\partial y} \approx \frac{p_{i,j+1,k} - p_{i,j,k}}{\Delta y}, \quad
\frac{\partial  \bm{p} }{\partial z} \approx \frac{p_{i,j,k+1} - p_{i,j,k}}{\Delta z}
\end{equation}
...
Y_{m,n} =
\begin{cases}
\frac{1}{\varDelta x}, &   p_x(i+1，j，k)， p_y(i，j+1，k)，p_z(i，j，k+1)に対応する成分 \\
-\frac{1}{\varDelta x}, &  p_x(i，j，k)， p_y(i，j，k)，p_z(i，j，k)に対応する成分\\
0, & \text{otherwise}
\end{cases}
\]
...
\section{基底空間構築手法}
スタッガード格子を用いた流体シミュレーションの，時刻$t$の流速データ$\bm{u_t}$と圧力データ$\bm{p}$を用いて，以下のスナップショット行列$\bm{S_u}$と$\bm{S}_p$を定義する．
 \[ \bm{S_u} = 
	 \begin{bmatrix}
...
\]
 \[ \bm{S}_p = 
	 \begin{bmatrix}
  	 \bm{p}_0 & \bm{p}_1 &\cdots  & \bm{p}_{T-1} \\
	\end{bmatrix}
\]
ここで，$\rm{x，y，z}$軸方向の分割数を$nx，ny，nz$$(nx = ny = nz)$とすると，$\bm{u_t}$と$\bm{p}$の次元はそれぞれ$3(nx+1)\times nx\times nx$，$nx\times nx \times nx$である．
...
まず，$n \times T$行列$\bm{S}$を以下のようにQR分解する．
\[
	\bm{S} = \bm{Q}\bm{R}
\]
ここで，行列$\bm{Q}$は$n \times n$直交行列であり，行列$\bm{R}$は$n \times T$上三角行列である．
その後，行列$\bm{R}$の第$T$行まで取り出し$T\times T$上三角行列にした後，以下のように特異値分解する．
\[
...
\ref{sec:Subspace}にて
$\bm{S}^{\mathsf T}\bm{S} = \bm{V}\bm{\Sigma}^2 \bm{V}^{\mathsf T}$
となることを利用して基底を求めることができることを紹介した．同様にしてQR分解後の特異値分解を利用して基底を求めることができる．この手法により，特異値分解する行列は各列に相関がある$n \times T$行列からQR分解後の$T \times T$上三角行列になり，空間計算量と行列の条件数が改善される．
...
\[
	\bm{u}_3  =  \bm{u}_2 - \bm{Y}\bm{p} 
\]
部分段階法では，中間子$\bm{u}_0$から$\bm{u}_3$までの4種類の速度ベクトルと，圧力ベクトルを計算する．それぞれのベクトルを部分空間に射影するため，対応したスナップショット行列が1つずつ必要である．中間子$\bm{u}_i$から得た基底を$\bm{U}_i$，$\bm{p}$から得られた基底を$\bm{P}$とする．
...
\end{align*}
ただし，$\bm{P}^{\mathsf T}\bm{A}^{-1}\bm{P} = (\bm{P}^{\mathsf T}\bm{A}\bm{P})^{-1}$であり，圧力ベクトルの次元数を$n$として$\bm{A}$は$n\times n$疎行列であり，$(\bm{P}^{\mathsf T}\bm{A}\bm{P})$は$r\times r$密行列である．密行列であることに注意が必要だが，逆行列を高速に求められるため，$\bm{\widetilde{A}} = (\bm{P}^{\mathsf T}\bm{A}\bm{P})^{-1}$とする．

部分空間法によるシミュレーション計算は，前処理として基底と部分空間に射影した行列を計算する．基底を計算するステップの計算量のボトルネックは，QR分解の$O(nT^2)$である．部分空間に射影した行列を計算するステップについては，射影前のそれぞれの行列が非常に疎であるため，基底の計算ほど時間はかからない．
...
\begin{figure}[htbp]
\caption{$密度誤差のボリュームデータの断面図$}
\label{fig:slice}
\centering
\begin{tabular}{cc}
\begin{minipage}[b]{0.45\linewidth}
\includegraphics[width=65mm]{images/dev1_100_slice.png}
\subcaption{既存手法，100フレーム目}
\end{minipage}

\begin{minipage}[b]{0.45\linewidth}
\includegraphics[width=65mm]{images/dev1_101_slice.png}
\subcaption{既存手法，101フレーム目}
\end{minipage}
\\
\begin{minipage}[b]{0.45\linewidth}
\includegraphics[width=65mm]{images/dev2_100_slice.png}
\subcaption{分割数2，100フレーム目}
\end{minipage}

\begin{minipage}[b]{0.45\linewidth}
\includegraphics[width=65mm]{images/dev2_101_slice.png}
...
今後の研究として，流れ場の不自然な変化を抑制する手法の適用が考えられる．部分空間を補間する手法を用いることで，部分空間の様子が滑らかに変化するようになり，不自然な変化を抑制することができると考えられる．

また，GPUを用いた大規模高速計算の併用などを用いた高速化が挙げられる．
...
A. Treuille, A. Lewis, and Z. Popovi\'{c}. Model reduction for real-time fluids. \textit{ACM Transactions on Graphics}, 25(3):826--834, 2006.T
