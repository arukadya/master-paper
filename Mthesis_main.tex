...
\subsection{部分空間への射影}
シミュレーションの前処理として，$n$次元ベクトル$\bm{x}$を$r$次元ベクトル$\bm{\widetilde{x}} $に変換する，$n \times r$行列$\bm{A}$を考える．$n \times r$行列$\bm{A}$を考える．$\bm{A}$は$\bm{A^{\mathsf T}} \bm{A} = \bm{I}$を満たすとすると，以下の2式が成り立つ．
 	\[
