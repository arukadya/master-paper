...
\end{bmatrix}
\]
ここで，固有ベクトル$\bm{v_i}$に対応する固有値を$\lambda_i$は，$\lambda_0 \ge \lambda_1 \cdots \ge \lambda_i \ge \lambda_{n-1} $を満たすとする．$\bm{S}\bm{S^T}$は対称行列になっているため，固有ベクトルは互いに直交し，$\bm{A^{-1}} = \bm{A^T}$となる．%また，$n \times n$行列$\bm{S^T}\bm{S}$ と$m\times m$行列$\bm{S}\bm{S^T}$の固有ベクトルは
...
%$\bm{A}$を用いて$\bm{u}$を$\bm{\tilde{u}}$に射影するとすると，$\bm{\tilde{u}} = \bm{A^T}\bm{u}$より，以下が成り立つ．
%\[
...
%	\bm{P^T}\bm{b} = (\bm{P^T}\bm{W}\bm{U_2})\bm{U_2^T}\bm{u_2}						&a\\
%	\bm{P^T}\bm{p} = (\bm{P^T}\bm{A^{-1}}\bm{P})\bm{P^T}\bm{b}						&b\\
%	\bm{U_3^T}\bm{u_3}  =  \bm{U_2^T}\bm{u_2} - (\bm{U_3^T}\bm{Y}\bm{P})\bm{P^T}\bm{p}&c
%\end{aligned}
%\]
\begin{align*}
 \bm{U_2^T}\bm{u_2}	& = (\bm{U_2^T}\bm{V}\bm{U_1})\bm{U_1^T}\bm{u_1} 					&\bm{\tilde{u_2}} 		&= \bm{\tilde{V}}\bm{\tilde{u_1}}	\\
